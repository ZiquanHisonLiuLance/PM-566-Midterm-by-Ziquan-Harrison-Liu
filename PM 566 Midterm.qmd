...
```
## Brief description: The aforementioned results indicated that the predominant percentage of flu vaccination sites publicize availability for the general populace (65.0%), followed by a significant portion aimed specifically at adults aged 65 and older (31.0%), while a mere minority (4.0%) explicitly advertised eligibility for younger adults or children, highlighting a provider emphasis on widespread accessibility with an additional focus on older adults.
```{r}
...
num_summary_tbl <- as.data.frame(t(num_summary)) %>%                                     # transpose so each row = variable
  mutate(var = rownames(.)) %>%                                             # attach missing counts per variable
  relocate(var)
#
kable(num_summary_tbl,
      caption = "Distribution summaries for numeric variables in `fluvac`") %>%  # publishable numeric summary  #
...
```
## Brief description: The results indicated that although geographic coordinates (latitude and longitude) correspond with the anticipated range of U.S. provider locations, the variable days since update displayed a broad distribution (median ≈454 days, maximum >800 days), implying potential issues regarding data timeliness. Additionally, the supply level variable demonstrates minimal variation, which may limit its interpretability in epidemiological analyses.
```{r}
...
```
## Brief description: The output below showed states with the highest counts of listed flu vaccine providers include [Top 10: CA, TX, FL, NY, PA,IL,OH,VA,NC,MI], reflecting high retail pharmacy density and population size. States/territories with the fewest listed providers tend to be smaller-population jurisdictions or territories.
```{r}
## checking in-stock status and stock recency
stock_summary <- fluvac %>%
  mutate(
    # Step 1. Coerce in_stock_flag into a clean logical variable                  #
    in_stock_clean = case_when(
      # if it's already logical TRUE/FALSE, keep it                              #
      in_stock_flag %in% c(TRUE, FALSE) ~ as.logical(in_stock_flag),             

      # if it's "true"/"false" as character or factor, convert to logical        #
      str_to_lower(str_trim(as.character(in_stock_flag))) == "true"  ~ TRUE,
      str_to_lower(str_trim(as.character(in_stock_flag))) == "false" ~ FALSE,

      # anything else becomes NA (shouldn't matter if you truly have no NA)     #
      TRUE ~ NA
    ),

    # Step 2. Human-readable label for reporting                                 #
    stock_status = if_else(
      in_stock_clean %in% TRUE,                      # rowwise: is this location in stock?         #
      "In stock",                                    # label if TRUE                                #
      "Not in stock"                                 # label if FALSE or NA                         #
    )
  ) %>%
  group_by(stock_status) %>%
  summarise(
    n_sites = n(),                                                   # number of locations in each status category          #
    median_days_since_update = median(days_since_update, na.rm = TRUE), # typical recency of supply reporting (days)        #
    .groups = "drop"
  )
#
kable(
  ...
  row_spec(
    1:nrow(stock_summary),
    background = c("#efedf5","white")
  )
```
## Brief description: The results showed that approximately 24.5% (49,713 locations) of flu vaccine providers reported being in stock, with a median reporting recency of 571 days. In contrast, the majority (152,927 locations) were classified as not in stock, with a median reporting recency of 451 days, underscoring significant variability and potential delays in stock status updates among sites.
```{r}
...
```
## Brief description:The histogram revealed that most flu vaccine supplier locations had not refreshed their stock status for several hundred days, particularly clustered around 450–600 days, indicating a lack of recent updates and likely deficiencies in the timely reporting of vaccine availability.
```{r}
...
p_violin_age
```
## Brief description: The violin plot illustrated that across all estimated eligible age categories (65+, all ages, and younger populations), stock updates are comparably old, with elongated right-skewed distributions indicating considerable delays in the latest reporting of vaccination availability.
```{r}
...
p_density_fresh
```
## Brief description: The density map revealed that most flu vaccine supplier sites had not refreshed stock information for several hundred days, with significant peaks at 450 and 570 days, underscoring considerable delays in reporting recency across several locales.
```{r}
...
p_state_line
```
## Brief description: The figure depicted the average duration since the last reported influenza vaccine stock update by U.S. state, revealing significant variation among states, with the majority falling within the range of approximately 450–500 days, thereby underscoring discrepancies in reporting timeliness that may indicate differences in provider practices or state-level surveillance systems.
